...
export default function App(){
  ...
  function handleLogo(){
    const now=Date.now()
    if(now-lastClick.current>3000) setAdminClicks(1); else setAdminClicks(c=>c+1)
    lastClick.current=now
    if(adminClicks+1>=5){ setShowAdmin(true); setAdminClicks(0) }
  }

  function handleFotos(e){
    const files=Array.from(e.target.files)
    if(files.length+fotosMoveis.length>3){ t('Máximo 3 fotos'); return }
    files.forEach(file=>{
      if(file.size>5*1024*1024){ t('Foto muito grande (max 5MB)'); return }
      const reader=new FileReader()
      reader.onload=(ev)=>{ setFotosMoveis(prev=>[...prev,{id:Date.now()+Math.random(),name:file.name,data:ev.target.result}]) }
  ...
  function handleFotoPerfil(e){
    ...
    if(!file) return
    if(file.size>5*1024*1024){ t('Foto muito grande'); return }
    const reader=new FileReader()
  ...
  function criarPedido(){
    ...
    const pedido={
      ...
      movel_nome:movelSel.nome,
      servico_tipo:servSel,
      servico_label:servSel==='novo'?'Montar Novo':servSel==='usado'?'Montar Usado':'Desmontar',
      valor_txt:valorTxt,
      valor_num:valorNum,
      ...
      bairro:form.bairro,
      rua:form.rua,
      numero_end:form.numero,
      data:form.data,
      horario:form.horario,
      fotos:fotosMoveis.map(f=>f.data),
      status:'AGUARDANDO_PAGAMENTO',
      created_at:new Date().toISOString()
    }
    const novos=[...pedidos,pedido]
    setPedidos(novos)
    localStorage.setItem('ccsp_pedidos',JSON.stringify(novos))
    setMovelSel({...movelSel,_ultimo:pedido})
    setFotosMoveis([])
    setView('pagamento')
    t('Pedido #'+pedido.numero+' criado!')
  }
    ...
    <div style={{minHeight:'100vh',background:'#F5F7FA',fontFamily:'Poppins'}}>
      <header style={{background:'#fff',padding:'12px 16px',display:'flex',justifyContent:'space-between',alignItems:'center',position:'sticky',top:0,zIndex:50,boxShadow:'0 2px 10px rgba(0,0,0,.05)'}}>
        ...
        <div style={{display:'flex',gap:8,alignItems:'center'}}>
          {!user && <><button onClick={()=>setView('login')} style={{background:'#0A2A6B',color:'#fff',border:'none',borderRadius:8,padding:'8px 12px',fontSize:12}}>Entrar</button><button onClick={()=>setView('cadastro')} style={{background:'#FF7A00',color:'#fff',border:'none',borderRadius:8,padding:'8px 12px',fontSize:12}}>Cadastro</button></>}
        ...
        <div style={{maxWidth:1100,margin:'0 auto',padding:16}}>
          ...
          {moveisBusca ? (
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))',gap:10,marginTop:16}}>
              {moveisBusca.map(m=>(
                <div key={m.id} style={{background:'#fff',borderRadius:16,padding:14,display:'flex',justifyContent:'space-between',alignItems:'center',cursor:'pointer',boxShadow:'0 4px 20px rgba(10,42,107,.08)'}} onClick={()=>{setCatSel({categoria:m.categoria}); setMovelSel(m); setServSel(null);}}>
            ...
            <>
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))',gap:12,marginTop:16}}>
                {catsFiltradas.map(cat=>(
                  <div key={cat.categoria} style={{background:'#fff',borderRadius:16,padding:16,textAlign:'center',cursor:'pointer',boxShadow:'0 4px 20px rgba(10,42,107,.08)'}} onClick={()=>{setCatSel(cat); setBusca('');}}>
      ...
      {movelSel && !movelSel._ultimo && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.5)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:100,padding:16}} onClick={()=>{setMovelSel(null); setServSel(null);}}>
          <div style={{background:'#fff',borderRadius:20,padding:20,maxWidth:480,width:'100%',maxHeight:'90vh',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{fontSize:11,background:'#E8EDF5',padding:'4px 8px',borderRadius:20,display:'inline-block',marginTop:8}}>{catSel?.categoria||movelSel.categoria}</div>
            ...
            {servSel && (
              <div style={{marginTop:16,background:'#F5F7FA',padding:14,borderRadius:12}}>
                <div style={{fontSize:12}}>Serviço selecionado:</div>
                <div style={{fontWeight:800}}>{movelSel.nome} - {servSel==='novo'?'Montar Novo':servSel==='usado'?'Montar Usado':'Desmontar'}</div>
                <div style={{fontWeight:800,fontSize:22,color:'#FF7A00',marginTop:6}}>{servSel==='novo'?movelSel.novo:servSel==='usado'?movelSel.usado:movelSel.desmontar}</div>
              </div>
            )}
            <button onClick={()=>{
              if(!user){ setView('cadastro'); t('Faça cadastro para continuar'); return }
              const el=document.getElementById('endereco-form'); if(el) el.scrollIntoView({behavior:'smooth'})
            }} disabled={!servSel} style={{width:'100%',marginTop:16,background:'#FF7A00',color:'#fff',border:'none',borderRadius:12,padding:'14px',fontWeight:700,opacity:servSel?1:0.5}}>
              {servSel ? `Confirmar serviço - ${servSel==='novo'?movelSel.novo:servSel==='usado'?movelSel.usado:movelSel.desmontar}` : 'Selecione um serviço'}
            </button>
            {user && user.tipo==='CLIENTE' && servSel && (
              <div id="endereco-form" style={{marginTop:20,borderTop:'1px solid #eee',paddingTop:16}}>
                ...
                <div style={{display:'grid',gap:8,marginTop:10}}>
                  <select value={form.cidade} onChange={e=>setForm({...form,cidade:e.target.value})} style={{padding:12,borderRadius:10,border:'1px solid #ddd'}}>
                    <option value="">Cidade (645 disponíveis)</option>{CIDADES_SP.map(c=><option key={c} value={c}>{c}</option>)}
                  </select>
                  ...
                  <div style={{marginTop:8,border:'2px dashed #E8EDF5',borderRadius:12,padding:12}}>
                    <label style={{fontWeight:700,fontSize:13}}>📸 Foto do móvel (opcional)</label>
                    <input type="file" accept="image/*" multiple onChange={handleFotos} style={{marginTop:6,width:'100%'}}/>
                    <div style={{display:'flex',gap:8,marginTop:8,flexWrap:'wrap'}}>
                      {fotosMoveis.map(f=>(
                        <div key={f.id} style={{position:'relative'}}>
                          <img src={f.data} style={{width:80,height:80,borderRadius:8,objectFit:'cover'}}/>
                          <button onClick={()=>setFotosMoveis(fotosMoveis.filter(x=>x.id!==f.id))} style={{position:'absolute',top:-6,right:-6,background:'#ff4444',color:'#fff',border:'none',borderRadius:'50%',width:20,height:20,fontSize:10}}>X</button>
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        <div style={{maxWidth:500,margin:'20px auto',padding:16}}>
          <div style={{background:'#fff',borderRadius:16,padding:20}}>
            ...
            <form onSubmit={(e)=>{
              ...
              if(usuarios.find(u=>u.email===email)){ t('Email já cadastrado'); return }
              if(fd.get('tipo')==='MONTADOR' && !fd.get('chave_pix')){ t('Chave PIX obrigatória para montador'); return }
              if(fd.get('tipo')==='MONTADOR' && !fotoPerfil){ t('Foto de perfil obrigatória para montador'); return }
              const novo={
                id:Date.now().toString(),
                tipo:fd.get('tipo'),
                nome:fd.get('nome'),
                ...
                cidade_atende:fd.get('cidade_atende'),
                telefone:fd.get('telefone'),
                chave_pix:fd.get('chave_pix')||'',
                foto_perfil:fotoPerfil,
                status:fd.get('tipo')==='MONTADOR'?'PENDENTE':'ATIVO'
              }
              const novos=[...usuarios,novo]
              setUsuarios(novos)
              localStorage.setItem('ccsp_usuarios',JSON.stringify(novos))
              setUser(novo)
              localStorage.setItem('ccsp_user',JSON.stringify(novo))
              setView('home')
              t('Cadastro ok!')
            }} style={{display:'grid',gap:10,marginTop:12}}>
              <select name="tipo" id="tipoCadastro" onChange={(e)=>{ const el=document.getElementById('montadorFields'); if(el) el.style.display=e.target.value==='MONTADOR'?'grid':'none' }} style={{padding:12,borderRadius:10,border:'1px solid #ddd'}}><option value="CLIENTE">Sou Cliente</option><option value="MONTADOR">Sou Montador</option></select>
              ...
              <div id="montadorFields" style={{display:'none',gap:10}}>
                <div style={{background:'#fff7ed',padding:12,borderRadius:10,border:'1px solid #FF7A00'}}>
                  <label style={{fontWeight:700,fontSize:12}}>Campos obrigatórios para Montador</label>
                  <select name="cidade_atende" required style={{width:'100%',marginTop:8,padding:12,borderRadius:10,border:'1px solid #ddd'}}><option value="">Cidade que atende (obrigatório)</option>{CIDADES_SP.map(c=><option key={c} value={c}>{c}</option>)}</select>
                  <div style={{marginTop:10}}>
                    <label style={{fontSize:12,fontWeight:600}}>📸 Foto de perfil (obrigatório - upload galeria)</label>
                    <input type="file" accept="image/*" onChange={handleFotoPerfil} style={{marginTop:6,width:'100%'}}/>
                    {fotoPerfil && <img src={fotoPerfil} style={{width:80,height:80,borderRadius:50,marginTop:8,objectFit:'cover'}}/>}
                  </div>
                  <input name="chave_pix" placeholder="Chave PIX no seu próprio nome (CPF, tel, e-mail ou aleatória) - OBRIGATÓRIO" style={{width:'100%',marginTop:10,padding:12,borderRadius:10,border:'1px solid #ddd'}}/>
                  <div style={{fontSize:10,opacity:.6,marginTop:4}}>A chave PIX deve estar no seu nome para receber os valores dos serviços</div>
                </div>
              </div>

              <button style={{background:'#FF7A00',color:'#fff',border:'none',borderRadius:12,padding:'14px',fontWeight:700}}>Criar conta</button>
              <button type="button" onClick={()=>setView('login')} style={{background:'none',border:'none',color:'#0A2A6B',fontSize:12}}>Já tenho conta</button>
            </form>
          </div>
        </div>
      )}
        ...
        <div style={{maxWidth:400,margin:'20px auto',padding:16}}>
          <div style={{background:'#fff',borderRadius:16,padding:20}}>
            ...
            <form onSubmit={(e)=>{
              ...
              if((email==='AndreSousa84'||email==='andre@contatocertosp.com.br')&&senha==='Contato@2026SP'){ const admin={id:'admin',tipo:'ADMIN',nome:'Andre'}; setUser(admin); localStorage.setItem('ccsp_user',JSON.stringify(admin)); setView('admin'); return }
              const u=usuarios.find(x=>x.email===email&&x.senha===senha)
              if(!u){ t('Inválido'); return }
              setUser(u); localStorage.setItem('ccsp_user',JSON.stringify(u)); setView(u.tipo==='CLIENTE'?'home':u.tipo==='MONTADOR'?'montador':'admin')
            }} style={{display:'grid',gap:10,marginTop:12}}>
      ...
      {view==='pagamento' && movelSel && movelSel._ultimo && (
        <div style={{maxWidth:480,margin:'20px auto',padding:16}}>
          <div style={{background:'#fff',borderRadius:16,padding:20,textAlign:'center'}}>
            ...
            <div style={{textAlign:'left',fontSize:13,background:'#fff7ed',padding:12,borderRadius:10}}>
              Pedido #{movelSel._ultimo.numero}<br/>{movelSel._ultimo.movel_nome} - {movelSel._ultimo.servico_label}<br/>Valor: <b>{movelSel._ultimo.valor_txt}</b><br/>Cidade: {movelSel._ultimo.cidade}<br/>Foto: {movelSel._ultimo.fotos&&movelSel._ultimo.fotos.length>0?`Sim (${movelSel._ultimo.fotos.length})`:'Não'}
            </div>
            <button onClick={()=>{
              const p=movelSel._ultimo; const msg=`Olá! Estou enviando o comprovante de pagamento do pedido Nº ${p.numero}.%0ACliente: ${p.cliente_nome}%0AServiço: ${p.movel_nome} - ${p.servico_label}%0AValor: ${p.valor_txt}%0ACidade: ${p.cidade}%0AFoto enviada: ${p.fotos&&p.fotos.length>0?'Sim ('+p.fotos.length+')':'Não'}`; window.open(`https://wa.me/${WHATSAPP}?text=${msg}`,'_blank'); const novos=pedidos.map(x=>x.id===p.id?{...x,status:'COMPROVANTE_ENVIADO'}:x); setPedidos(novos); localStorage.setItem('ccsp_pedidos',JSON.stringify(novos)); t('Comprovante enviado!')
            }} style={{width:'100%',marginTop:16,background:'#FF7A00',color:'#fff',border:'none',borderRadius:12,padding:'14px',fontWeight:700}}>📲 ENVIAR COMPROVANTE WHATSAPP</button>
            <button onClick={()=>setView('home')} style={{background:'none',border:'none',marginTop:12,color:'#0A2A6B'}}>Voltar</button>
          </div>
        </div>
      )}

      <button style={{position:'fixed',bottom:80,right:20,zIndex:90,background:'#0A2A6B',color:'#fff',borderRadius:50,padding:'12px 18px',boxShadow:'0 4px 20px rgba(0,0,0,.3)',display:'flex',alignItems:'center',gap:8,fontSize:13,fontWeight:600,border:'none',cursor:'pointer'}} onClick={()=>t('App em breve na Play Store! Por enquanto use pelo navegador e adicione à tela inicial')}>📲 Baixar App</button>

      <footer style={{background:'#0A2A6B',color:'#fff',padding:24,marginTop:40,textAlign:'center',fontSize:11}}>
        <div style={{fontWeight:800}}>CONTATO CERTO SP</div>
        <div>Atendimento: (18) 99148-8302 • 645 cidades SP • 77 móveis</div>
      </footer>
    </div>
  )
}
